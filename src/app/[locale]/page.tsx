...
export default async function HomePage({ params }: Params) {
  ...
  return (
    <>
      {/* 1. Hero */}
      <Section className="grid-bg pt-12 sm:pt-20">
          ...
          {automate.map((item, i) => {
            ...
            return (
              <li key={item.title} className="flex flex-col rounded-xl border border-border bg-card p-6">
                ...
                <h3 className="mt-4 font-sans text-lg font-semibold tracking-tight">{item.title}</h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-muted-foreground">{item.desc}</p>
                <p className="mt-4 border-t border-border pt-3 font-mono text-xs font-semibold text-primary">{item.metric}</p>
              </li>
            );
          })}
        </ul>
      ...
      <Section id="how" tone="muted">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.4fr_1fr_1fr]">
          <div className="rounded-lg border-2 border-primary bg-card p-7 shadow-lg sm:p-9">
            <div className="flex items-center gap-3">
              <span className="font-mono text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("how.audit.step")}</span>
            ...
            <p className="mt-2 font-mono text-4xl font-semibold tracking-tight text-primary">
          ...
          {(["build", "run"] as const).map((k) => (
            <div key={k} className="rounded-lg border border-border bg-card/60 p-7">
              <span className="font-mono text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t(`how.${k}.step`)}</span>
              <h3 className="mt-3 text-2xl font-semibold">{t(`how.${k}.title`)}</h3>
              <p className="mt-1 font-mono text-2xl font-semibold tracking-tight text-primary">{t(`how.${k}.price`)}</p>
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {(["oz", "cris"] as const).map((p) => (
            <div key={p} className="flex gap-5 rounded-xl border border-border bg-card p-6">
              ...
              <div>
                <h3 className="font-sans text-xl font-semibold tracking-tight">{t(`who.${p}.name`)}</h3>
      ...
      {/* 7. Pricing teaser */}
      <Section>
        <div className="grid items-center gap-6 rounded-lg border border-border bg-card p-7 sm:p-10 md:grid-cols-[2fr_auto]">
          <div>
